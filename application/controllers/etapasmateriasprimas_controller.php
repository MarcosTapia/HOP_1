...
class Etapasmateriasprimas_controller extends CI_Controller {
...
//        $this->gatewayRest =  $cadena;
    ...
    function mostrarEtapasMateriasPrimas() {
        if ($this->is_logged_in()){
            $url = RUTAWS.'etapasmateriasprimas/obtener_etapasmateriasprimas.php';
            $ch = curl_init($url);
            ...
            if ($datos->{'estado'}==1) {
                $data = array(
                    'etapasmateriasprimas'=>$datos->{'etapasmateriasprimas'},
                    'usuarioDatos' => $this->session->userdata('nombre'),
                    'fecha' => $fechaIngreso,
                    'nombre_Empresa'=>$this->nombreEmpresaGlobal,
                    'permisos' => $this->session->userdata('permisos'),
                ...
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('etapasmateriasprimas/adminEtapasMateriasPrimas_view',$data);
                $this->load->view('layouts/pie_view',$data);
            } else {
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('principal_view',$data);
    ...
    function nuevoEtapaMateriaPrima() {
        if ($this->is_logged_in()){
            $dt = new DateTime("now", new DateTimeZone('America/Mexico_City'));
            $fechaIngreso = $dt->format("Y-m-d H:i:s"); 
            ...
            $this->load->view('layouts/headerAdministrador_view',$data);
            $this->load->view('etapasmateriasprimas/nuevoEtapaMateriaPrima_view',$data);
            $this->load->view('layouts/pie_view',$data);
        } else {
            redirect($this->cerrarSesion());
        }
    ...
    function nuevoEtapaMateriaPrimaFromFormulario() {
        if ($this->is_logged_in()){
            if ($this->input->post('submit')){
                ...
                $ch = curl_init(RUTAWS.'etapasmateriasprimas/insertar_etapamateriaprima.php');
                ...
                redirect('/index.php/etapasmateriasprimas_controller/mostrarEtapasMateriasPrimas'); 
            }
        } else {
            redirect($this->cerrarSesion());
        }
